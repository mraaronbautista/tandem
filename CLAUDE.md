...
Three persistent destinations — **Today** (the task list, default), **Rentals**, **Reports** — are tracked in `TaskBoard.jsx`'s `activeTab` state and rendered via a responsive nav (`.task-board-nav` in `App.css`): a fixed bottom tab bar below 900px, a sticky left sidebar above it — one markup, two layouts via a media query, the same approach `Modal.jsx` already uses for its own mobile/desktop split. This replaced a narrow centered column that left large unused gutters on wide screens. `RentalsView.jsx` and `EodReportsList.jsx` are plain tab content now, not modals — no `onClose`, nothing to dismiss, you just switch tabs; both are exclusively reached this way (no other call site).

Everything else is a one-shot action, not a place you browse an ongoing state of — those stay behind the floating "+" button (`NewTaskForm.jsx`), which doubles as a speed-dial menu whenever an `extraActions` array is passed to it. That array (priorities, vault, submit report, nudge) is built and scoped per-viewer in `TaskBoard.jsx`'s `quickActions` (e.g. "Submit report" only appears for Aaron, "Nudge" only for Ada) — `NewTaskForm.jsx` itself has no opinion on who sees what. The header stays minimal — working-status indicator + a single ⚙️ settings button (`SettingsMenu.jsx`: notifications toggle, theme, sign out) — separate from the new nav.
