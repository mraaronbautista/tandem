...
**Calendar** (`RentalCalendar.jsx`): shows one unit at a time, switched via a row of unit tabs (`.rental-unit-tabs`, one click to switch — a `<select>` dropdown originally, replaced because it took a scroll-and-choose on every switch) rendering a standard 7-column month grid — not a multi-unit Gantt-style timeline, which an earlier version tried and had to be scrapped (it needed a wider modal, which fought the mobile CSS reset and broke the layout on phones). `.rental-calendar` is width-capped at the `min-width: 900px` breakpoint so the day cells (`aspect-ratio: 1`) don't stretch into oversized squares on the wide desktop sidebar layout the way they would filling the full tab-content width. `check_out` on `rental_bookings` is the *last occupied day, inclusive* — not a hotel-style departure day — so a single-day booking has `check_out = check_in`.
