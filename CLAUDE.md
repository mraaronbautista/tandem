...
The whole panel (`.rentals-panel` — `.tab-panel` plus this class) is width-capped at the `min-width: 900px` breakpoint, not just the calendar grid — an earlier version capped `.rental-calendar` alone, which fixed the grid's day cells (`aspect-ratio: 1`) stretching into oversized squares but left the period-tabs/month-nav rows above it spanning the full desktop sidebar-layout width, a mismatched, lopsided look. Capping the whole panel keeps every row at one consistent width instead.
