...
The whole panel (`.rentals-panel` — `.tab-panel` plus this class) is width-capped **and centered** (`margin: 0 auto`) at the `min-width: 900px` breakpoint, not just the calendar grid — an earlier version capped `.rental-calendar` alone, which fixed the grid's day cells (`aspect-ratio: 1`) stretching into oversized squares but left the period-tabs/month-nav rows above it spanning the full desktop sidebar-layout width, a mismatched, lopsided look. A later pass capped the whole panel but without centering it, which left it flush against the left edge of `.task-board-content` with a large empty gap on the right — just as unbalanced-looking on a wide screen as being uncapped, even though the cap itself (verified via `getComputedStyle`) was working correctly. Centering makes the cap actually read as intentional.
