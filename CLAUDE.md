...
A side-feature bolted onto the task board: `RentalsView.jsx` (the "Rentals" tab — see UI structure below) tracks occupancy and finances for Ada's furnished-rental business. Hardcoded to `company: 'awa'` throughout the frontend even though the `rental_company` enum also has `'azu'` (Ada's mom's separate, unrelated rental business) — `azu` was added to the schema so it wouldn't require a migration later, but no UI exists for it yet; it's deliberately out of scope. All the `rental_*` tables use the same `is_member()` RLS as everything else — mutual visibility, no per-row ownership. Three period-tabs: Calendar, Financials, Overview.

Only `.rental-month-grid` itself is width-capped (`max-width: 420px`) at the `min-width: 900px` breakpoint — this went through two worse attempts first. Capping `.rental-calendar` (toolbar + grid together) fixed the grid's day cells (`aspect-ratio: 1`) stretching into oversized squares, but also squeezed the unit-tabs row down to the same narrow width, wrapping the four unit tabs onto two rows instead of sitting side by side. Capping the whole `.tab-panel` instead (verified via `getComputedStyle` to genuinely apply) avoided that, but with no centering it sat flush against the left edge of the wide desktop content area, and centering it just shrank everything uniformly, reintroducing the unit-tab wrapping. Capping only the grid lets the toolbar above it (period-tabs, month-nav, unit-tabs) use the full desktop content width — room for every unit tab in one row — while the grid alone stays a comfortable size.

**Calendar** (`RentalCalendar.jsx`): shows one unit at a time, switched via a row of unit tabs (`.rental-unit-tabs`, one click to switch — a `<select>` dropdown originally, replaced because it took a scroll-and-choose on every switch) rendering a standard 7-column month grid — not a multi-unit Gantt-style timeline, which an earlier version tried and had to be scrapped (it needed a wider modal, which fought the mobile CSS reset and broke the layout on phones). `check_out` on `rental_bookings` is the *last occupied day, inclusive* — not a hotel-style departure day — so a single-day booking has `check_out = check_in`.
