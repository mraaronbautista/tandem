...
const REPORT_COLUMNS = 'id, submitted_by, period, report_date, minutes_logged, body, created_at, updated_at'

// One row per (submitted_by, period, report_date) — appends bodyChunk to
// the existing row's body and overwrites minutesLogged (not sums it) if
// a bucket already exists for this period, via the upsert_eod_report SQL
// function (plain .upsert() can't express "old body + new chunk").
export async function submitEodReport(period, reportDate, { bodyChunk, minutesLogged }) {
