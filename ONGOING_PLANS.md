...
- [ ] Verify the payroll-cadence migration and filtering UI against the live Supabase project (schema block applied, cadence editable/persisted via `StaffProfileForm.jsx`, period stepper correctly scoping real `time_entries` rows for at least one non-biweekly cadence), then update `CLAUDE.md`'s Property manager section, remove this completed plan, commit, and push the final phase.
