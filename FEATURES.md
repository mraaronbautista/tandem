...
Five persistent tabs for Ada/Aaron: **Today**, **Rentals**, **Reports**, **Board** (which folds together Cork Board and Inbox as two switchable sections), and — when staff exist — the Hours/staff admin screen. The floating "+" is contextual: Today opens a tabbed Task Tools modal (New task / Bulk / Priorities), Rentals opens Add booking, Reports opens Submit report for Aaron, and Board focuses the new-pin composer; Staff has no ambiguous add action. The header holds the working-status indicator, the nudge icon, and Settings. Vault now has a dedicated row in Settings alongside theme, notifications, default timezone, sign out, and the in-app guide.
